...
window.ResearchFlows = (() => {
  function cloud(c,seed,S=3){
    ...
    state.at=t=>{if(!Number.isFinite(t)||t<0||t>S)throw new RangeError('Time must be between zero and three doublings.');const width=2**(t-S),ceiling=height*2**(-t);return {t,width,ceiling,fixed:points.filter(p=>p.u<=width).length,scale:points.filter(p=>p.u<=width&&p.z<ceiling).length,meanFixed:c*2**t,meanScale:c};};
